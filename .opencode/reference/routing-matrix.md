...
| QA/deployment | Test strategy, verification sweeps, release prep, deployment docs, rollback planning, infra delivery, CI or rollout work | `qa-validation`, `devops-platform` | bounded validation/evidence -> `quick`; release/platform/risk-heavy -> `deep` | `review-work`, `git-master`, `agent-browser`, `dev-browser` | Keep bounded validation, evidence capture, and finish-pass checks in the lighter `quick` lane. Escalate to `deep` when the work centers on release orchestration, platform changes, rollback posture, environment risk, or other higher-risk deployment concerns. Use `Prometheus` when release prep needs a structured plan. Use `Oracle` for risk review before deployment. Use `Explore` to locate tests, pipelines, and environment docs. Use `agent-browser` or `dev-browser` as optional upstream helpers for live browser execution, accessibility snapshots, screenshots, or browser evidence capture when verification depends on it. Use `Context7` for official platform, registry, or deployment-tool reference detail, and use `Librarian` for broader standards or source-of-truth lookup. Use `git-master` for branch, commit, and release-history tasks. Use `review-work` after substantial implementation or validation changes. | `none` | Existing project in place, or greenfield -> `workspace/{project-name}-{domain}` |
...
These packs are explicitly tiered as `planned` adjacent references. They are not primary routes, they are not present-tense support claims, and they should only be surfaced after the dominant bucket and primary pack are already chosen.
